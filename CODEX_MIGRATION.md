...
| Business folder | Старая `.business/` модель заменена на `business/` | Инструкция использует видимую папку; приватность теперь обеспечивается `/business/` в `.gitignore`. |
| Live metrics | Добавлен маршрут `business/life-metrics.md` | Меняющиеся цифры должны браться из свежего источника по маркеру `LIVE`, а не хардкодиться в документах. |
| Raw sources | Добавлен маршрут `business/raw/` | Исходники отделены от обработанных выводов и не входят в обязательное стартовое чтение. |
