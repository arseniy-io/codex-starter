...
Стартовый шаблон для проектов, где Codex используется как основной coding agent. Клонируешь репозиторий, открываешь его в Codex, проходишь онбординг и получаешь: бизнес-контекст, `AGENTS.md`, рабочий цикл `plan → implement → verify → retro`, личный контекст, методологии и базовые guardrails безопасности.
...
- `ai-clone/` и `mastery/` сначала tracked как плейсхолдеры. После заполнения личными данными или внешними материалами реши: приватный репозиторий или снятие с tracking.
