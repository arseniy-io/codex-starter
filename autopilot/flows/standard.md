...
Закрой восемь вопросов и запиши состояние каждого:

1. Блокирует: рабочее название и суть продукта. Закрой `project_identity`.
2. Блокирует: основной пользователь и его проблема. Закрой `primary_audience` и `standard_user_problem`.
3. Блокирует: ближайший результат, первый scope, границы MVP и следующий шаг. Закрой `nearest_result`, `first_scope`, `standard_mvp_boundary` и `next_step`.
4. Блокирует, если есть пользовательский путь: главное действие и путь до заявки, покупки или другого результата. Закрой `standard_user_path`, при отсутствии пути допустим `not_applicable`.
5. Не блокирует: цены, тарифы или правила доступа. Закрой `standard_pricing`.
6. Не блокирует: доверие, страхи и возражения. Закрой `standard_objections`.
7. Не блокирует: текущие или предполагаемые каналы привлечения. Закрой `standard_channels`.
8. Блокирует: есть ли в ближайшем MVP роли, персональные данные, платежи, API, файлы, export/audit, multi-tenant, marketplace или AI-решения. Закрой `standard_deep_trigger_screen`.

Если сильный deep-триггер входит в ближайший MVP, нельзя закрыть его как `unknown_for_now`. Перейди на `deep` либо вместе с пользователем убери эту возможность из MVP.
