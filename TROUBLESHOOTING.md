...
### У меня осталась старая `.business/`

Новая модель starter использует видимую папку `business/`. Для миграции смотри [`MIGRATION_BUSINESS_FOLDER.md`](./MIGRATION_BUSINESS_FOLDER.md).

Коротко:

1. Переименуй `.business/` в `business/`.
2. Проверь, что в `.gitignore` есть `/business/`.
3. Добавь `business/life-metrics.md` для живых цифр.
4. Добавь `business/raw/README.md` для исходников.
...
### Preview остановился на изменённом или неизвестном файле

Это защитная остановка, а не ошибка удаления. Ничего не удалено. Если файл важен, оставь его как пользовательский и перенеси из служебной зоны Starter. Если известный файл Starter был осознанно изменён, сначала реши, нужно ли сохранить его содержание в финальных документах. Не подменяй контрольную сумму вручную только ради прохождения проверки.

После решения снова запусти:

```powershell
python autopilot/finalization/finalize.py --root . preview
```

### Старый `preview_id` больше не принимается

Значит, после preview изменилось дерево проекта, decisions-файл или финальные документы. Создай новый preview, покажи его пользователю и получи новое подтверждение. Старое согласие намеренно не переносится.

### Очистка прервалась

Не запускай интервью заново и не удаляй остатки вручную. Сначала проверь сохранённую стадию:

```powershell
python autopilot/finalization/finalize.py --root . status
```

Затем повтори `apply` с тем же подтверждённым `preview_id`. Уже завершённые операции записаны в журнал и выполняются безопасно повторно.

### Проект без Git

Preview потребует отдельное подтверждение, потому что простого отката через Git нет. Если не уверен, сначала сделай копию папки. Сам Starter не запускает `git init`, commit, push и не меняет remote.
